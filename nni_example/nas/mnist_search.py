...
@model_wrapper
class ConvCustom(nn.Module):
    def __init__(self, nIn, nOut):
        super().__init__()
        self.net = torch.nn.Sequential(
                nn.Conv2d(nIn, nOut, kernel_size=nn.ValueChoice([1, 3, 5]),
                    stride=nn.ValueChoice([1, 2])),
                nn.ReLU(True))
...
class Net(nn.Module):
    def __init__(self, nIn, nOut):
        ...
        self.layers.append(ConvCustom(1, 32))
        # self.conv1 = nn.Conv2d(1, 32, 3, 1)
        # LayerChoice is used to select a layer between Conv2d and DwConv.

        self.layers.append(ConvCustom(32, 64))
        self.layers.append(DepthwiseSeparableConv(32, 64))
    ...
    def forward(self, x):
        ...
        import pdb
        pdb.set_trace()
        x = F.max_pool2d(self.layers(x), 2)
...
def evaluate_model(model_cls):
    ...
    train_loader = DataLoader(MNIST('/home/shenghao/dataset', download=True, transform=transf), batch_size=64, shuffle=True)
    test_loader = DataLoader(MNIST('/home/shenghao/dataset', download=True, train=False, transform=transf), batch_size=64)
